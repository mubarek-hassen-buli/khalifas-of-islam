...
const CaliphCard: React.FC<CaliphCardProps> = ({ 
  ...
  return (
    ...
    >
      <div className="absolute -inset-0.5 bg-gradient-to-r from-gold-light via-gold to-gold-dark rounded-xl blur opacity-25 group-hover:opacity-50 transition duration-300"></div>
      <div className="relative bg-islamic-charcoal border border-gold/10 rounded-xl p-6 h-full flex flex-col">
        <div className="w-24 h-24 mx-auto mb-4 relative">
          <div className="absolute inset-0 rounded-full bg-gradient-to-r from-gold-light to-gold-dark blur-sm opacity-20 group-hover:opacity-50 transition-opacity duration-300"></div>
          <img
            src={image}
            alt={name}
            className="w-full h-full object-cover rounded-full border-2 border-gold/30 relative z-10"
          />
        </div>
        ...
        <div className="mt-4 text-center">
          <span className="text-xs text-gold-light cursor-pointer story-link">
            Learn more
          </span>
        </div>
      </div>
