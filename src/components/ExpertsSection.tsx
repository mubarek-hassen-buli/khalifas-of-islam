...
const ExpertsSection = () => {
  ...
  return (
    <section
      id="experts"
      className="py-20 bg-islamic-charcoal relative overflow-hidden perspective"
    >
      <div className="absolute inset-0 islamic-pattern opacity-5"></div>

      {/* Glowing orbs for background effect */}
      <div
        className="absolute top-1/4 left-1/4 w-40 h-40 bg-gold-light/5 rounded-full blur-3xl transform-3d"
        style={{ transform: `translateZ(-150px)` }}
      ></div>
      <div
        className="absolute bottom-1/3 right-1/4 w-60 h-60 bg-gold-light/5 rounded-full blur-3xl transform-3d"
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12">
          ...
          >
            <div className="mb-2 opacity-80">
              <p className="text-gold-light font-arabic text-2xl">
                علماء الإسلام
              </p>
            </div>
            <h2
              className="text-3xl md:text-4xl font-bold font-cinzel gold-gradient mb-4 opacity-0 animate-fade-in"
              style={{
                animationDelay: "0.1s",
            ...
            <p
              className="text-white/70 max-w-xl opacity-0 animate-fade-in"
              style={{ animationDelay: "0.2s", animationFillMode: "forwards" }}
            >
              Explore the works of these esteemed scholars who documented the
              lives and legacies of the Four Righteous Caliphs through their
              meticulous historical records.
            </p>
          </div>

          <Link
            to="/#experts"
            className="mt-4 md:mt-0 px-6 py-2 border border-gold-light/30 text-gold-light hover:bg-gold-light/10 rounded-full transition-colors text-sm opacity-0 animate-fade-in transform-3d"
            style={{
              animationDelay: "0.3s",
              animationFillMode: "forwards",
            ...
            >
              ...
              <div className="relative bg-islamic-blue/10 border border-gold/10 rounded-xl p-8 flex flex-col items-center h-full">
                ...
                <div className="text-center mb-4 opacity-90">
                  <p className="text-gold-light font-arabic text-xl">
                    {expert.arabicName}
                  </p>
                </div>

                <h3 className="text-2xl font-cinzel font-semibold text-center gold-gradient mb-1">
                  {expert.name}
                </h3>
                ...
                <p className="text-white/50 text-xs mb-4">
                  {expert.specialty} • {expert.era}
                </p>

                <p className="text-white/80 text-sm mb-6 text-center line-clamp-3">
                  {expert.description}
                </p>
