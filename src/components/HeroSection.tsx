    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden islamic-pattern">
      ...
      <div className="container mx-auto px-4 md:px-8 z-10 pt-16 md:pt-0">
        <div className="flex flex-col items-center justify-center text-center">
          ...
          <p className="text-gold-light uppercase tracking-widest text-sm mb-4 opacity-0 animate-fade-in" style={{ animationDelay: '0.2s', animationFillMode: 'forwards' }}>
            The Legacy of The
          </p>
          ...
          <h1 ref={titleRef} className="text-5xl md:text-7xl lg:text-9xl font-bold font-cinzel mb-6 gold-gradient relative">
            {Array.from("KHILAFAH").map((letter, i) => (
              <span key={i} className="letter inline-block opacity-0 translate-y-8 transition-all duration-500" style={{ transitionDelay: `${i * 0.05}s` }}>
                {letter}
              </span>
            ))}
          </h1>
          
          <p className="max-w-2xl text-white/80 mb-8 text-lg opacity-0 animate-fade-in" style={{ animationDelay: '1s', animationFillMode: 'forwards' }}>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-float opacity-0 animate-fade-in" style={{ animationDelay: '1.5s', animationFillMode: 'forwards' }}>
        <ArrowDown size={24} className="text-gold-light" />
      </div>
    </section>
  );
};
